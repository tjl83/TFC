...
namespace TestModules.NetModule
{
...
#pragma warning restore 618
        ...
        private void listenForConnection(TcpListener pListener)
        {
            ...
            while (isOnline)
            {
                try
                {
                    ...
                    //Start SIGNALNEWCLIENT
                    IPAddress foo = ((IPEndPoint)nextClient.Client.RemoteEndPoint).Address;
                    String ip = foo.ToString();
                    Console.Out.WriteLine("Client from " + ip + " initiated contact.");
                    //End SIGNALNEWCLIENT
                }
                catch (Exception e)
                {
        ...
        private void clientListener(TcpClient clSocket)
        {
            ...
            while (isOnline && clSocket.Connected)
            {
                ...
                //Start SIGNALNEWMESSAGE
                IPAddress foo = ((IPEndPoint)clSocket.Client.RemoteEndPoint).Address;
                String ip = foo.ToString();
                ...
                {
                    Console.Out.Write(byt);
                }
                Console.Out.Write("\n");
                //End SIGNALNEWMESSAGE
            }
        }
